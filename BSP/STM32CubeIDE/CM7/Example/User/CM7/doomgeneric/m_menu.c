...
void M_ReadSaveStrings(void)
{
    ...
    {
        unsigned int flash_pos = SAVES_FLASH_OFFSET + i*1*1024*1024; // 1MiB reserved per save file

        // Try to check if the savefile exists
        static char buffer[1024];
        if(BSP_QSPI_Read(0, buffer, flash_pos, 1024) != BSP_ERROR_NONE)
        {
        	assert(false);
        }

        if (strcmp(buffer, "STM32DOOMSAVE") == 0)
        {
            static char tempbuf[80];
            DEH_snprintf(tempbuf, 80, "DOOMSAVE%d", i);
            M_StringCopy(savegamestrings[i], tempbuf, SAVESTRINGSIZE);
            LoadMenu[i].status = 1;
        }
        else
        {
...
void M_SaveSelect(int choice)
{
    ...
    static char tempbuf[80];
    DEH_snprintf(tempbuf, 80, "DOOMSAVE%d", choice);
    M_StringCopy(savegamestrings[choice], tempbuf, SAVESTRINGSIZE);
