...
static boolean ConvertMusBuffer(should_be_const byte* musdata, int len, void** buf, int* buf_len)
{
    MEMFILE* instream;
    MEMFILE* outstream;
    void* outbuf;
    ...
    if (result == 0)
    {
        ...
        *buf_len = outbuf_len;
        *buf = Z_Malloc(outbuf_len, PU_LEVEL, 0);
...
static void *I_OPL_RegisterSong(should_be_const void *data, int len)
{
    ...
    else
    {
        ...
        ConvertMusBuffer(data, len, &buf, &buf_size);
    }

    MEMFILE* instream = mem_fopen_read(buf, buf_size);

    result = MIDI_LoadFile(instream);
