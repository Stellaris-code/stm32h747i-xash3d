...
size_t entity_map_counter;
__attribute__((section(".extram.entnames"))) const char* entity_map_names[8192];
__attribute__((section(".extram.entfuncs"))) entity_callback_t entity_map_funcs[8192];

EntityMap::EntityMap(const char* name, entity_callback_t func)
{
	entity_map_names[entity_map_counter] = name;
	entity_map_funcs[entity_map_counter] = func;
	++entity_map_counter;
}

entity_callback_t GetEntitySpawnCallback(const char* name)
{
	for (int i = 0; i < entity_map_counter; ++i)
	{
		if (strcmp(entity_map_names[i], name) == 0)
			return entity_map_funcs[i];
	}
	return NULL;
}
