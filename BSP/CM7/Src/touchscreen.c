...
uint32_t ts_colors[24] = {UTIL_LCD_COLOR_BLUE, UTIL_LCD_COLOR_GREEN, UTIL_LCD_COLOR_RED, UTIL_LCD_COLOR_CYAN, UTIL_LCD_COLOR_MAGENTA, UTIL_LCD_COLOR_YELLOW,
    ...
    for(i = 0; i < hTS->Width/40; i++)
    {
      for(j = 0; j < hTS->Height/40; j++)
      {
        if(((x_new_pos > 40*i) && (x_new_pos < 40*(i+1))) && ((y_new_pos > 40*j) && (y_new_pos < 40*(j+1))))
        {
          ...
          UTIL_LCD_FillRect(40*i, 40*j,40,40, ts_colors[(touchscreen_color_idx++ % 24)]);
