...
char *P_SaveGameFile(int slot)
{
    ...
    M_snprintf(basename, 32, SAVEGAMENAME "%d", slot);
...
uint8_t* save_data_base;
uint8_t* save_data_ptr;

static int read_savedata(void* buf, int size)
{
	memcpy(buf, save_data_ptr, size);
	save_data_ptr += size;
	return size;
}

static int write_savedata(void* source, int size)
{
	memcpy(save_data_ptr, source, size);
	save_data_ptr += size;
	return size;
}

static int savedataptr_tell()
{
	return save_data_ptr - save_data_base;
}
