...
__attribute__((section(".ramd3"))) byte		consistancy[MAXPLAYERS][BACKUPTICS];
...
extern uint8_t* save_data_base;
extern uint8_t* save_data_ptr;

extern int savegameindex;

void __attribute__((optimize("O0"))) G_DoLoadGame (void)
{
    int savedleveltime;
	 
    gameaction = ga_nothing; 

    unsigned int flash_pos = SAVES_FLASH_OFFSET + savegameindex*1*1024*1024; // 1MiB reserved per save file


    save_data_base = Z_Malloc(1024*1024, PU_STATIC, 0); // 1MiB of savedata reserved
    save_data_ptr = save_data_base;

    int ret;
    if((ret = BSP_QSPI_Read(0, save_data_base, flash_pos, 1024*1024)) != BSP_ERROR_NONE)
    {
    	assert(false);
    }
...
void __attribute__((optimize("O0"))) G_DoSaveGame (void)
{ 
    char *savegame_file;
    ...
    unsigned int flash_pos = SAVES_FLASH_OFFSET + savegameslot*1*1024*1024; // 1MiB reserved per save file


    //save_data_base = 0x90000000 + 64*1024*1024; // Save files in FLASH, after the 128M reserved for IWADs
    save_data_base = Z_Malloc(1024*1024, PU_STATIC, 0); // 1MiB of savedata reserved
    save_data_ptr = save_data_base;

	int ret;
    //ret = BSP_QSPI_DisableMemoryMappedMode(0);
    //assert(ret == BSP_ERROR_NONE);

    for (int i = 0; i < 4; ++i)
    {

		if((ret = BSP_QSPI_EraseBlock(0, flash_pos + i*128*1024,BSP_QSPI_ERASE_128K)) != BSP_ERROR_NONE)
		{
		  assert(false);
		}
    ...
    P_WriteSaveGameHeader("STM32DOOMSAVE");
    ...
    // MI11: write the save file back in flash
    if((ret = BSP_QSPI_Write(0, save_data_base, flash_pos, save_data_ptr - save_data_base)) != BSP_ERROR_NONE)
    {
    	assert(false);
    }
