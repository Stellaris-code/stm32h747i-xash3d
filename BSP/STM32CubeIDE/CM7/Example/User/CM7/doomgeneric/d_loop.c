...
static __attribute__((section(".ramd3"))) ticcmd_set_t ticdata[BACKUPTICS];
...
boolean D_InitNetGame(net_connect_data_t *connect_data)
{
     ...
     || M_CheckParm("-privateserver") > 0)
    {
        ...
        NET_SV_AddModule(&net_stm_module);
    ...
    else
    {
        ...
        i = vcp_connected(); // attempt to autojoin only when serial port is connected
        ...
        if (i > 0)
        {
            net_stm_module.InitClient();
            addr = net_stm_module.ResolveAddress(myargv[i+1]);
    ...
    if (addr != NULL)
    {
        ...
        NET_Log("D_InitNetGame: Connected to %s\n", NET_AddrToString(addr));
...
void TryRunTics (void)
{
	...
	if (I_GetTime() / ticdup - entertic > /*MAX_NETGAME_STALL_TICS*/5)
	{
	    return;
	}
