...
void Host_Frame( float time )
{
	...
	Host_InputFrame ();  // input frame
		host.inputframetime = t1 - Sys_DoubleTime();
	Host_ClientBegin (); // begin client
	host.clientbegintime = t1 - Sys_DoubleTime();
	Host_GetCommands (); // dedicated in
	host.getcmdtime = t1 - Sys_DoubleTime();
	Host_ServerFrame (); // server frame
	host.svframetime = t1 - Sys_DoubleTime();
	Host_ClientFrame (); // client frame
	// clientframe makes up for like 90% of the frame time
	host.clframetime = t1 - Sys_DoubleTime();
...
//	Con_Reportf( "Sys_LoadLibrary: Loading xash.dll - ok\n" );
	...
	*/
	...
	extern void D_SCDump(void);

	Cmd_AddCommand( "exec", Host_Exec_f, "execute a script file" );
	Cmd_AddCommand( "memlist", Host_MemStats_f, "prints memory pool information" );
	Cmd_AddCommand("scdump", D_SCDump, "dump surfcache information");
