...
{
	if( UI_AddTouchButtonToList )
	{
		UI_AddTouchButtonToList( name, texture, command, color, flags );
	}
}
...
qboolean UI_LoadProgs( void )
{
	...
	gameui.hInstance = (void*)0xdeadbeef; // some dummy value
	...
	GetMenuAPI(&gameui.dllFuncs, &gpEngfuncs, gameui.globals);
	// make local copy of engfuncs to prevent overwrite it with user dll
	memcpy(&gpExtendedfuncs, &gExtendedfuncs, sizeof(gExtendedfuncs));
	memset(&gameui.dllFuncs2, 0, sizeof(gameui.dllFuncs2));
	GetExtAPI(MENU_EXTENDED_API_VERSION, &gameui.dllFuncs2, &gpExtendedfuncs);
	...
	else // otherwise, fallback to old and deprecated extensions
	{
		...
		UI_AddTouchButtonToList = (ADDTOUCHBUTTONTOLIST)COM_GetProcAddress( gameui.hInstance, "AddTouchButtonToList" );
		if( UI_AddTouchButtonToList )
		{
