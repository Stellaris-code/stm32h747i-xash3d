...
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
#define RX_BUFFER_MAX_WRITE_INDEX (APP_RX_DATA_SIZE - CDC_DATA_HS_MAX_PACKET_SIZE)
...
static int8_t CDC_Init_HS(void)
{
  ...
  vcp_tx_fifo.data = UserTxBufferHS;  // Use the buffer generated by Cube
  vcp_tx_fifo.wr = 0;
  vcp_tx_fifo.rd = 0;
  vcp_tx_fifo.lb = 0;
  vcp_rx_fifo.data = UserRxBufferHS;  // Use the buffer generated by Cube
  vcp_rx_fifo.wr = 0;
  vcp_rx_fifo.rd = 0;
  vcp_rx_fifo.lb = 0;
...
static int8_t CDC_Receive_HS(uint8_t* Buf, uint32_t *Len)
{
  ...
  USBD_CDC_SetRxBuffer(&hUsbDeviceHS, vcp_rx_fifo.data + vcp_rx_fifo.wr);
...
void vcp_init ()
{
  while (vcp_tx_fifo.data == 0);		// Wait until the CDC library calls CDC_Init_HS, by checking for a non-null buffer pointer
}
...
void vcp_service ()
{
  ...
  // wait 250ms, timeout otherwise
  uint32_t start_tick = HAL_GetTick();
  while (hcdc->TxState != 0)
  {
	  if (HAL_GetTick() - start_tick > 250)
	  {
		  serial_timeout = 1;
		  return;
	  }
  ...
  //if (hcdc->TxState == 0)
  {
    ...
    if (cap != 0)  // The FIFO is empty : return immediately
    {
      if (cap < 0)  // The FIFO contents wrap-around
      {
        ...
        USBD_CDC_SetTxBuffer(&hUsbDeviceHS, &vcp_tx_fifo.data[vcp_tx_fifo.rd], APP_TX_DATA_SIZE - vcp_tx_fifo.rd);
        USBD_CDC_TransmitPacket(&hUsbDeviceHS);
        vcp_tx_fifo.lb = 0;    // Lock the tail’s data
      }
      else  // No wrap-around : send the whole FIFO
      {
        USBD_CDC_SetTxBuffer(&hUsbDeviceHS, &vcp_tx_fifo.data[vcp_tx_fifo.rd], cap);
        USBD_CDC_TransmitPacket(&hUsbDeviceHS);
